...
struct pyramic* pyramicInitializePyramic() {
	...
	p->h2f_lw_axi_master = mmap(NULL, p->h2f_lw_axi_master_span, PROT_READ | PROT_WRITE, MAP_SHARED, p->fd_dev_mem, p->h2f_lw_axi_master_ofst);
    ...
    p->reserved_memory = mmap(NULL, RESERVED_MEMORY_SIZE_PHY, PROT_READ | PROT_WRITE, MAP_SHARED, p->fd_dev_mem, RESERVED_MEMORY_OFFSET_PHY);
	if (p->reserved_memory == MAP_FAILED) {
		free(p);
		return NULL;
	}

    p->output_memory = mmap(NULL, SEC_MEMORY_SIZE_PHY, PROT_READ | PROT_WRITE, MAP_SHARED, p->fd_dev_mem, SEC_MEMORY_OFFSET_PHY);
